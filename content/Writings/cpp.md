...
|              | 现在C++特性  | auto、decltype、constexpr、结构化绑定、概念               |
| 性能优化     | 右值引用     | 移动语义、完美转发                                        |
|              | 编译优化     | 内联、常量折叠、返回值优化                                |
| 底层编程     | 内存模型     | 字节对齐、内存布局                                        |
|              | 位操作       | 位运算、位域                                              |
...
- [C++ 语言参考 | Microsoft Learn](https://learn.microsoft.com/zh-cn/cpp/cpp/cpp-language-reference?view=msvc-170)
...
2. 继承(零阶抽象)
   - 实现代码的复用
   - 父类到子类, 是抽象到一般的过程
3. 多态(二阶抽象) - 实现接口的复用 - 通过*抽象*的接口, 得到*一般*的结果 - 分类 - 静态: 函数重载 & 模板类 - 动态: `virtual`
   ![[cpp 2025.excalidraw#^frame=cpp_property|400]]
...
定义对象时, 分配存储空间(房间) - 该空间有其地址(位置)和名称(对象名)两个属性, 地址是唯一的, 但是名称可以不唯一(某个命名空间内是唯一即可),遂可通过引用来取别名 - 该空间的内容可通过2种方式获取, 一: 通过对象名直接获取, 二: 通过地址获取时候需要使用符号`(*)`来获取
...
3. usage attention
   ...
   1. **变量作用域**：Lambda 表达式不能捕获非静态局部变量或由 `register` 关键字声明的变量。
   1. **类型推断**：Lambda 表达式的返回类型可以由编译器自动推断，但有时可能需要显式指定返回类型，尤其是在 Lambda 表达式体中包含多条语句时。
   1. **性能考虑**：虽然 Lambda 表达式提供了便利，但过度使用或不当使用（如捕获大量变量）可能会影响程序性能。
   1. **可读性**：Lambda 表达式应该保持简洁，如果 Lambda 表达式过于复杂，可能需要考虑将其转换为普通函数。
   1. **线程安全**：如果 Lambda 表达式用于多线程环境，需要确保捕获的变量是线程安全的。
   1. **异常安全性**：Lambda 表达式在捕获变量时，如果这些变量的类型不保证异常安全性，那么 Lambda 本身也可能不具备异常安全性。
   1. **内存分配**：对于通过值捕获的大型对象，Lambda 表达式可能会增加内存使用，因为每个 Lambda 表达式实例都会复制这些对象。
...
![[cpp 2025.excalidraw#^frame=data_structure_dequeue|Dequeue Principle|300]]

- A deque is somewhat recursively defined: internally it maintains a double-ended queue of *chunks* of fixed size. Each chunk is a vector, and the queue (“map” in the graphic below) of chunks itself is also a vector[^4].
