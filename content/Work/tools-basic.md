...
1. 查看python替换版本信息：`update-alternatives --list python`
   ...
   如果出现 update-alternatives：error：no alternatives for
   python则表示Python 的替换版本尚未被update-alternatives 命令识别。想解决这个问题，我们需要更新一下替 换版本，将Python2.7和python3.5放入其中。执行以下命令：
...
| 获取多个提交         | `git fetch --unshallow` or `git fetch --depth=10`         |
| 远程强制覆盖本地代码 | `git fetch miui`<br>`git reset --hard miui/master-w-qcom` |

1. 技巧
   1. 创建本地分支并链接到远程库
      1. `repo start dev --all`
         //建立本地与远程分支的链接，不需要知道远程分支名是什么就可以建立对应的链接
...
1. remote rejected
   1. `git push --no-thin ***`
   2. `git repack -ad && rm -f $(git rev-parse --git-path objects/info/alternates)`
...
1. 直接命令行抓取
   - 教程:
     [system-tracing](https://perfetto.dev/docs/quickstart/android-tracing#recording-a-trace-through-the-cmdline)
   ```bash
...
1. 安装Android SDK `sudo apt update && sudo apt install android-sdk`

1. 开始抓取systrace一般执行以下命令就可以:
...
1. 查看systrace打开以下网站，加载之前生成的html就可以了`chrome://tracing`
   > [!warning] 注意
   >
...
- 用途: 用于编解码
- 工作流程 ![[tools 2024.excalidraw|800]]
...
> - 表达式：`tag:priority ...`
> - tag:感兴趣的标记
> - priority:最低优先级e.g.: `adb logcat ActivityManager:I MyApp:D *:S`
