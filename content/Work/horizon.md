...
- 手机坐标轴
  ![[phone-axes 2025.excalidraw|手机坐标轴|300]]
- 输出的数据
  - 报告设备坐标系中重力的方向和大小
...
- 校准注意事项
  ...
  1. XXX 需要尽量保持远景, 这样被拍摄物体尽量是与x*y平面平行, 保证计算出的$\theta*{xy}$能够尽量与使用成片基于cv计算出的转角是同一个平面
...
![[horizon 2025.excalidraw|800]]

## Fov Cal

$w = sensor\_active\_size\_width$
$h = sensor\_active\_size\_height$
$\frac{w}{h} = \frac{4}{3}$
$D = h * eis\_crop\_factor$
$\frac{a}{b} = \frac{16}{9}$
$a^2 + b^2 = D^2$

$\frac{a}{w} = 0.8*0.653 = 0.5224$

![[horizon 2025.excalidraw#^frame=fov_cal|200]]
