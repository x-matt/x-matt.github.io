...
- **长焦**拍照的场景下预览画面的稳定性对用户拍照体验的影响愈发的明显，尤其体现在**月亮模式**，预览画面的抖动目标难以捕获。
...
![[zoomeis 2025.excalidraw#^frame=软件框架图|ZoomEIS框架图]]
...
![[zoomeis 2025.excalidraw#^frame=算法流程图|EISNode中，处理流程图]]
...
![[zoomeis 2025.excalidraw#^frame=默认裁切逻辑|默认的Pipeline裁切逻辑|800]]
...
3. _为了保证最终生成的图片解析力不会损失太多，方案中将仅仅利用原先交由P2模块crop的那一部分进行防抖，P1部分的逻辑保持不变_
4. 样例说明：
   1. Capture(4:3) - 2x
...
![[zoomeis 2025.excalidraw#^frame=中心裁切|中心裁切|700]]

![[zoomeis 2025.excalidraw#^frame=包含eis shift的裁切|按照 eis-cropInfo 进行裁切|700]]
...
2. 难点
   1. 9-in-1的sensor，ISZ的开启倍率为3x，4-in-1的sensor，ISZ的开启倍率则为2x，ISZ在达到开启倍率之后，还是会根据环境的亮暗来动态on/off，这就导致交给P2的可裁切区域也会随之发生跳变，因此EIS若启用动态margin功能，就会出现跳变前cropFactor为0.5，跳变后变为0.1的现象，画面切换瞬间会有明显抖动。
...
![[zoomeis 2025.excalidraw#^frame=zoomEis pipeline|zoomEIS pipeline|900]]
